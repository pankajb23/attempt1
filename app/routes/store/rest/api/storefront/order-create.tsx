...
import { CART_LINES_ADD, CART_LINES_UPDATE, CART_DISCOUNT_CODES_UPDATE, VARIANT_PRICES } from 'app/routes/store/graphql/mutations';
...
const ADD = async (storefront, cartId, cartDetails, payloadProducts) => {
  ...
  const toAddGraphQl = await storefront.graphql(CART_LINES_ADD, {
    variables: {
      cartId: cartId,
      lines: products,
    },
...
const UPDATE = async (storefront, cartId, cartDetails, payloadProducts) => {
  ...
  const toUpdateGraphQl = await storefront.graphql(CART_LINES_UPDATE, {
    variables: {
      cartId: cartId,
      lines: products,
    },
...
const ADD_OFFER = async (shop, offerId, cartId, discountCodes, cartDetails, productPricing) => {
  ...
  const discountCodeUpdates = await storefront.graphql(CART_DISCOUNT_CODES_UPDATE, {
    variables: {
      cartId: cartId,
...
const GET_PRODUCT_PRICING = async (storefront, cartDetails) => {
  const productPricing = await Promise.all(
    cartDetails.map(async (product) => {
      ...
      try {
        const response = await storefront.graphql(VARIANT_PRICES, {
          variables: {
            pid: pid,
          },
        });
        ...
        if (response.ok) {
          ...
          const selectedVariantPrice = productData.variants.nodes.find((variant) => variant.id === product.merchandise.id).price.amount;
...
export const action: ActionFunction = async ({ params, request }) => {
  ...
  const response = await storefront.graphql(CART_DETAILS, {
    variables: {
      cartId: cartId,
    },
  });
  ...
  // do it async.
  void ADD_OFFER(shop, offerId, cartId, discountCodes, cartDetails, productPricing);
