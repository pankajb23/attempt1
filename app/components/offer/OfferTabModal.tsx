...
function RowData({ allOffers, openPage }) {
    ...
    const areAllSelectedOnesActive = selectedResources.length === 0 ? false : selectedResources.map(Number)
        .every((idx) => ordersWithId[idx - (page - 1) * pageSize].status === "active");

    const areAllSelectedOnesDraft = selectedResources.length === 0 ? false : selectedResources.map(Number)
        .every(idx => ordersWithId[idx - (page - 1) * pageSize].status === "draft");
    ...
    const promotedBulkOptions = [
        {
            content: "Enable",
            onAction: async () =>
                await ToggleOffers(selectedOfferIds, "active", modalsAndStoreId.storeId, setModalsAndStoreId, handleSelectionChange),
            disabled: areAllSelectedOnesActive,
        },
        {
            content: "Disable",
            onAction: async () =>
                await ToggleOffers(selectedOfferIds, "draft", modalsAndStoreId.storeId, setModalsAndStoreId, handleSelectionChange),
                ...
                <IndexTable.Cell>
                    <BlockStack gap="100">
                        {<div style={{ width: '30px' }}>
                            <Button dataPrimaryLink={true} removeUnderline={false} variant="monochromePlain" fullWidth={false} size="micro" textAlign='left' >{wrap(offer.offerName)}</Button>
                        </div>
                        }
                        {
                            <div >
                                <Text as="p" variant="bodySm" > {offer.offerType}</Text>
                            </div>
                        }
                    </BlockStack>
                </IndexTable.Cell>
                <IndexTable.Cell><Badge tone={offer.status ==="active" ? "success":"info"}>{offer.status}</Badge></IndexTable.Cell>
        ...
        <>
            <Layout.Section>
                <Card padding="100">
                    ...
                    />{
                        allOffers.length > 0 ? <RowData allOffers={allOffers} openPage={openPage} /> :
