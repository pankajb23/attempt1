...
export default function UpsellFunnelDiscount() {
    ...
    return (    
        <Card roundedAbove="sm">
            <BlockStack gap="400">
                <Text as="h4" variant="headingSm">Upsell funnel discount</Text>
                <Controller
                    control={control}
                ...
                />
                {
                    isEnabled && (
                        <Controller
                            ...
                            render={({ field: { onChange, value } }) => (
                                <ChoiceList
                                    title={<Text as="h4" variant="headingSm">Discount type</Text>}
                                    choices={[
                                        { label: "Percentage of Fixed value", value: "percentageOrFixedValue" },
                ...
                }
                {
                    isEnabled && (
                        <Controller
                            ...
                            render={({ field: { onChange, value } }) => (
                                <TextField
                                    label={<Text as="p" variant="bodySm" fontWeight="bold">Minimum cart value for discount to apply</Text>}
                                    value={value}
                                    type="number"
                                    onChange={onChange}
                                    autoComplete="off"
                ...
                }
                {
                    isEnabled && (
                        <BlockStack gap="200">
                            <Text as="h4" variant="headingSm">Discount combinations</Text>
                            {
                                
                                discountType === "percentageOrFixedValue" && (
                                    <Text as="p" variant="bodySm">This product discount can be combined with</Text>
                                )
                            }
                            {
                                discountType === "freeShipping" && (
                                    <Text as="p" variant="bodySm">This shipping discount can be combined with</Text>
                                )
                            }
                        </BlockStack>
                    )
