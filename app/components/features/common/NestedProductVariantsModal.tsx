...
export default function NestedProductVariantsModal({ allVariants, modalId }) {
    ...
    const groupRowsByGroupKey = useCallback(
        (variants: ProductTags[], groupKey: keyof ProductTags, resolveId: (groupVal: string) => string) => {
            let position = -1;
    ...
    const handleParentSelection = useCallback((groupProducts: ProductRow[], isSelected: boolean) => {
    ...
    const rowMarkup = Object.keys(groupedAndFilteredProducts).map((color, index) => {
        ...
        return (
            <Fragment key={productId}>
                ...
                {products.map(
                    ({ pid, label, variant, priceVariation, currency, position }, rowIndex) => (
                        <IndexTable.Row
                            rowType="child"
                            key={rowIndex}
                            id={pid}
