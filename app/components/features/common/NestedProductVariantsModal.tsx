...
export default function NestedProductVariantsModal({ allVariants, modalId }) {
  ...
  const groupRowsByGroupKey = useCallback(
    (
      variants: ProductTags[],
      groupKey: keyof ProductTags,
      resolveId: (groupVal: string) => string
    ) => {
      let position = -1;
      const groups: Groups = variants.reduce(
        (groups: Groups, product: ProductTags) => {
  ...
  const groupedAndFilteredProducts = useMemo(() => {
    const allGroups = groupRowsByGroupKey(
      allVariants,
      'label',
  ...
  const visibleProductsCount = useMemo(() => {
    return Object.values(groupedAndFilteredProducts).reduce(
  ...
  const { selectedResources, allResourcesSelected, handleSelectionChange } =
    useIndexResourceState(flattenedFilteredProducts);
  ...
  const handleParentSelection = useCallback(
    (groupProducts: ProductRow[], isSelected: boolean) => {
      ...
      const newSelection = isSelected
        ? selectedResources.filter((id) => !productIds.includes(id))
        : [...new Set([...selectedResources, ...productIds])];
  ...
  const rowMarkup = Object.keys(groupedAndFilteredProducts).map(
    (color, index) => {
      const {
        products,
        position,
        id: productId,
      } = groupedAndFilteredProducts[color];
      ...
      return (
        <Fragment key={productId}>
          ...
          {products.map(
            (
              { pid, label, variant, priceVariation, currency, position },
              rowIndex
            ) => (
              <IndexTable.Row
                rowType="child"
                ...
                onClick={() => {
                  const newSelection = selectedResources.includes(pid)
                    ? selectedResources.filter((id) => id !== pid)
                    : [...selectedResources, pid];
                  handleSelectionChange(
